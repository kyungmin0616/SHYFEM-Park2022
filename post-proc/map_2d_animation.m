...
for kz=4*8:4*18
    ...
    tar_time=datenum(ECMWF.mttime(kz,1));
    
    I=find(CMEMS.mttime==tar_time);
...
%     colb=colorbar;
%     title(colb,'m')
%     axis([-83.5 -78 30 33]) % axis criterian
%     graph_handle= gca;
%     graph_handle.YTickLabel = strcat(graph_handle.YTickLabel, '^{\circ}');
%     graph_handle.XTickLabel = strcat(graph_handle.XTickLabel, '^{\circ}');
%     caxis([clw chi])
...
%     
    h1=subplot(1,2,1);
    pcolorjw(ECMWF.mtLON, ECMWF.mtLAT, sqrt(ECMWF.mtu(:,:,kz).^2 + ECMWF.mtv(:,:,kz).^2)); hold on
    h = quiver(ECMWF.mtLON(1:skp:end,1:skp:end), ECMWF.mtLAT(1:skp:end,1:skp:end),ECMWF.mtu(1:skp:end,1:skp:end,kz),ECMWF.mtv(1:skp:end,1:skp:end,kz),1.5,'w','LineWidth',2);
    ...
    title(colb,'m/s')
    axis([-82 -73 24 36]) % axis criterian
    title([datestr(ECMWF.mttime(kz), 'yyyy-mm-dd HH:MM:SS')])
    graph_handle= gca;
    graph_handle.YTickLabel = strcat(graph_handle.YTickLabel, '^{\circ}');
    graph_handle.XTickLabel = strcat(graph_handle.XTickLabel, '^{\circ}');
    xlabel('Longitude')
    ...
    saveas(h,sprintf('/Users/kpark350/Ga_tech/Projects/SHYFEM/Output/SenExp/Figures_StomSurge/OSM2022/Images/ani_%d.png',kz));
